import { DeviceType, fetchUser, setAccessToken, signinWithSocial } from '@/apis/auth.api';
...
export const useAppBridge = () => {
  ...
  const handleSocialLogin = async (data: SocialLoginRequest) => {
    ...
    try {
      ...
      const providerCode =
        typeof data.provider === 'string' ? getProviderCode(data.provider) : (data.provider as 'K' | 'N' | 'G' | 'A');
      ...
      const signinResponse = await signinWithSocial({
        ...
        deviceToken: data.fcmToken ?? localStorage.getItem(LOCAL_STORAGE_KEYS.FCM_TOKEN),
  ...
  const handleSetAccessToken = async (data: Record<string, string>) => {
    ...
    if (!data.accessToken) {
      const authRoutes = routeConfig.filter((route) => route.auth === true).map((route) => route.path);
