...
export const usePaymentHandlers = () => {
  ...
  const handlePointChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const totalAmount = usePaymentStore.getState().items.reduce((total, item) => total + item.price * item.amount, 0);
    ...
    try {
      ...
      usePaymentStore.setState((state) => ({
        items: state.items.map((item) =>
          item.ss_idx.toString() === cartId ? { ...item, amount: adjustedCount } : item
        )
      }));
    } catch (error) {
