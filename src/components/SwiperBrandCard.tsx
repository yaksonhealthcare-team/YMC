...
export const SwiperBrandCard = ({ className, onBrandClick, selectedBrandCodes }: SwiperBrandCardProps) => {
    ...
    >
      <div className="flex pb-2 pr-5">
        {displayedBrands.map((brand, index) => (
          <div key={`${brand.code}-${index}`} className={index === displayedBrands.length - 1 ? 'pr-5' : ''}>
            <BrandCard
              name={brand.name}
              brandSrc={brand.imageUrl ?? ''}
              onClick={() => onBrandClick(brand.code, brand.name)}
              selected={selectedBrandCodes ? selectedBrandCodes?.includes(brand.code) : false}
            />
          </div>
        ))}
      </div>
