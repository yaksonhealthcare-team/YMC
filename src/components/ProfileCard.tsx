...
const ProfileCard = ({ type, name, profileImageUrl, description, grade }: ProfileCardProps) => {
  const styles = STYLE_VARIANTS[type];

  return (
    <div className={`${styles.background} p-5 rounded-2xl shadow-md flex justify-between items-center`}>
      <div>
        <p className={`font-b text-18px ${styles.text}`}>
          {name} {grade}
        </p>
        {description && <p className={`${styles.description} text-14px ${styles.text}`}>{description}</p>}
      </div>
      <ProfileImage
