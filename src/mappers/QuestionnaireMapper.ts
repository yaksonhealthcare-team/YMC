...
export class QuestionnaireMapper {
  ...
  static toQuestionnaireResultEntity(dto: QuestionnaireResultResponse): QuestionnaireResult {
    return {
  ...
  static toQuestionnaireResultEntities(dtos: QuestionnaireResultResponse[]): QuestionnaireResult[] {
    return dtos.map((dto) => this.toQuestionnaireResultEntity(dto));
  }

  static toQuestionnaireFormValues(questions: QuestionnaireResult[]): QuestionnaireFormValues {
    ...
    questions.forEach((question) => {
      ...
      if (question.answer_type === 'T') {
        ...
        formValues[`${questionId}_text`] = question.options[0]?.answer_text || '';
        ...
        // 주관식 답변이 있는 경우
        const textAnswer = question.options.find((opt) => opt.option_type === '2')?.answer_text;
        if (textAnswer) {
