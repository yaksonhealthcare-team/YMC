...
import { Question, QuestionFieldName, QuestionnaireFormValues, QuestionnaireType } from '@/entities/user/model/questionnaire.types';
...
export const useQuestionnaire = ({ type, returnPath = '/', returnText = '메인 홈으로' }: UseQuestionnaireProps) => {
  ...
  const handleFieldChange = (fieldName: QuestionFieldName, value: QuestionnaireFormValues[QuestionFieldName]) => {
    setFormValues((prev) => ({
      ...prev,
      [fieldName]: value
    }));
  ...
  const { totalQuestions, currentQuestionNumber } = useMemo(() => {
    ...
    while (idx < questions.length) {
      ...
      if (Array.isArray(answer) && answer.length > 0) {
        const selectedOption = question.options.find((opt) => opt.csso_idx === answer[0].csso_idx);

        if (selectedOption?.next_cssq_idx) {
          // 다음 질문으로 건너뛰기
          const nextIdx = questions.findIndex((q) => q.cssq_idx === selectedOption.next_cssq_idx);
          if (nextIdx !== -1) {
  ...
  const handleNext = useCallback(async () => {
    ...
    if (currentIndex < questions.length - 1) {
      ...
      if (Array.isArray(currentValue) && currentValue.length > 0) {
        const selectedOption = currentQuestion.options.find((opt) => opt.csso_idx === currentValue[0].csso_idx);

        if (selectedOption?.next_cssq_idx) {
          const targetIndex = questions.findIndex((q) => q.cssq_idx === selectedOption.next_cssq_idx);
          if (targetIndex !== -1) {
  ...
  }, [questions, currentIndex, formValues, submitMutation, navigate, returnPath, returnText, showToast]);
