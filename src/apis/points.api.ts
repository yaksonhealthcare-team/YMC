...
export const fetchPointHistories = async ({ page, sort }: { page: number; sort: 'C' | 'D' }) => {
  const { data } = await axiosClient.get<HTTPResponse<{ total_point: string; result: PointHistoryResponse[] }>>(
    '/points/history',
    {
      params: {
        page: page,
...
export const fetchPoints = async () => {
  const { data } = await axiosClient.get<HTTPResponse<{ total_point: string }>>('/points/points');
