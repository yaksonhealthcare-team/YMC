...
const PaymentCompletePage = () => {
  ...
  const itemsArray = Array.isArray(state.items) ? state.items : state.items ? [state.items] : [];
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="p-5 border-b-8 border-gray-50">
        <div className="flex items-center gap-1 mb-4">
          ...
          <span className="text-primary font-sb text-16px">{itemsArray.length}건</span>
        </div>
        {itemsArray.map((item) => (
          <PaymentItemCard
            key={item.p_idx}
            item={{
              ...
              branchType: item.branch.code === '0' ? '전지점 사용가능' : item.branch.name,
              duration: 0,
              price: Number(state.amount_info.total_amount),
              sessions: Number(item.sessions),
              name: item.branch.code === '0' ? '전지점 사용가능' : item.branch.name,
      ...
      <FixedButtonContainer className="bg-white">
        <div className="flex gap-2 w-full">
          <Button variantType="line" sizeType="m" onClick={() => navigate('/', { replace: true })} fullWidth>
            홈으로 돌아가기
          </Button>
          <Button variantType="primary" sizeType="m" onClick={handleNavigate} fullWidth>
