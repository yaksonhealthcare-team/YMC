  ...
  return (
    ...
    >
      <div className={'flex justify-between items-center'}>
        ...
        {!canceled && <CheckIcon htmlColor={selected ? '#F37165' : '#DDDDDD'} />}
      </div>
      {canceled ? (
        ...
        <div className={'flex gap-2 items-center'}>
          ...
          <p className={'text-12px text-gray-500'}>{`${item.branchName} 사용 가능`}</p>
        </div>
      )}
    </div>
  );
...
const PaymentCancelPage = () => {
  ...
  if (!payment || isLoading) return <LoadingIndicator className="min-h-screen" />;
          ...
          <div className={'flex flex-col overflow-scroll p-5'}>
            ...
            <ul className={'mt-10 space-y-4'}>
                  ...
                  <PaymentCancelItemCard
                    ...
                    onClick={() => {
                      if (selectedItems.includes(item.index)) {
                        setSelectedItems((prev) => [...prev].filter((id) => id !== item.index));
                      } else {
              ...
              {!isGuideMessageLoading && paymentCancelMessage && (
                <p className={'text-14px text-gray-500'}>{paymentCancelMessage}</p>
              )}
            </ul>
          </div>
      ...
      return (
        <div className={'flex flex-col h-full justify-between overflow-hidden'}>
          <div className={'flex flex-col overflow-scroll p-5'}>
            ...
            <p className={'font-b text-20px mt-2'}>{'취소 사유를 말씀해주세요.'}</p>
            <div className={'mt-10'}>
              <textarea
                className={
            ...
            {!isGuideMessageLoading && paymentCancelMessage && (
              <p className={'text-14px text-gray-500 mt-4'}>{paymentCancelMessage}</p>
            )}
          </div>
          <div className={'border-t border-gray-100 px-5 pt-3 pb-8'}>
            <Button
              ...
              disabled={reason.length < MIN_REASON_LENGTH || reason.length > MAX_REASON_LENGTH || isOfflinePayment}
            >
              {'취소하기'}
            </Button>
          </div>
