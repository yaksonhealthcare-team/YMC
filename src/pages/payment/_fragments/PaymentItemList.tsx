...
const PaymentItemCard = ({
  ...
  <div className={'flex flex-col p-5 border border-gray-200 rounded-2xl'}>
    <p className={`${canceled ? 'text-gray-500' : 'text-primary'} text-14px font-m`}>{status}</p>
    ...
    {canceled && category === 'membership' && onClickShowCancelHistory && (
      <Button className={'mt-4'} variantType={'gray'} sizeType={'s'} onClick={onClickShowCancelHistory}>
        {'취소 내역 보기'}
      </Button>
    )}
  </div>
    ...
    <ul className={'flex flex-col space-y-4'}>
      {payment.items.map((item, idx) => (
        <li key={`${item.index}-${idx}`}>
          <Membership category={payment.category} item={item} onClickCancelHistory={onClickShowCancelHistory} />
        </li>
      ))}
    </ul>
  );
