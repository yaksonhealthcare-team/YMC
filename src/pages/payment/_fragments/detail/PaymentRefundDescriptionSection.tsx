...
const PaymentRefundDescriptionSection = ({ payment }: { payment: PaymentHistoryDetail }) => {
  ...
  const price = payment.items.reduce((acc, curr) => acc + curr.cancel.canceledPrice, 0);
  const points = payment.items.reduce((acc, curr) => acc + curr.cancel.usedPoint, 0);
  ...
  return (
    <div className={'flex flex-col'}>
      ...
      <div className={'flex justify-between mt-3 items-center'}>
        ...
        <p className={'text-14px font-sb'}>{payment.totalPrice.toLocaleString()}원</p>
      </div>
      <div className={'flex justify-between mt-3 items-center'}>
      ...
      <div className={'flex justify-between mt-4 items-center'}>
        ...
        <p className={'text-20px font-b text-gray-700'}>{price.toLocaleString()}원</p>
      </div>
    </div>
  );
};
