...
const PaymentInfo = ({ state }: PaymentInfoProps) => {
    ...
    return (
      <div className="px-5 py-6">
        <p className="text-gray-700 font-sb text-16px">
          신용카드 ({state.cardPaymentInfo?.cardName} / {state.cardPaymentInfo?.installment})
        </p>
      </div>
    );
  }
  ...
  if (state.paymentMethod === 'VBANK') {
    const bankName = state.vbankInfo?.bankCode ? BANK_CODE_MAP[state.vbankInfo.bankCode] || '확인불가' : '확인불가';
        ...
        <div className="px-5 py-6 flex flex-col gap-4">
          ...
          <div className="py-5 px-4 bg-gray-50 rounded-[20px] flex flex-col gap-3">
            <div className="flex justify-between">
              ...
              <span className="font-sb text-14px text-gray-700">{bankName}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-m text-14px text-gray-500">계좌번호</span>
              <span className="font-sb text-14px text-gray-700">{state.vbankInfo?.account || '확인불가'}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-m text-14px text-gray-500">예금주</span>
              <span className="font-sb text-14px text-gray-700">{state.vbankInfo?.accountName || '확인불가'}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-m text-14px text-gray-500">입금기한</span>
              <span className="font-sb text-14px text-error">{state.vbankInfo?.limitDate || '확인불가'}</span>
            </div>
          </div>
