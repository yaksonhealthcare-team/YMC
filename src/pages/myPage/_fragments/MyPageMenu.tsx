...
const MyPageMenu = () => {
  ...
  return (
    <nav className="bg-white rounded-[20px] border border-gray-100 p-5 space-y-4" aria-label="마이페이지 메뉴">
      {menuItems.map((item) => (
        <button
          type="button"
          key={item.id}
        ...
        >
          ...
          <CaretRightIcon className="w-3 h-3 text-gray-900" aria-hidden="true" />
        </button>
      ))}
    </nav>
  );
