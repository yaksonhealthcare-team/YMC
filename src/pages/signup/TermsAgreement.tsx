...
export const TermsAgreement = () => {
  ...
  const handleOnClickNext = async (event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        <LoadingIndicator />
        <p className="mt-4 text-16px font-medium text-[#212121]">본인 인증 정보를 처리 중입니다...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col px-5 pt-5 pb-7 gap-10">
      ...
      <div className="flex flex-col gap-6">
        ...
        >
          ...
          />
          <span className="font-semibold text-14px text-primary">전체약관 동의</span>
        </div>
...
const AgreementItem = ({ title, checked, onChange, onDetail }: AgreementItemProps) => {
  return (
