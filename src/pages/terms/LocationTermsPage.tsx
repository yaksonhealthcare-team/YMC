...
const LocationTermsPage = () => {
  ...
  const { data: termsData, isLoading } = useTermsByCategory(3); // 3: 위치기반 서비스 이용약관
  ...
  const categoryTerms = termsData?.terms?.find((category) => category.terms_category_idx === '3');
    ...
    <div className={'px-5 py-4'}>
      <div className="mb-6">
        <h1 className={'font-b text-24px text-gray-900'}>
          {terms?.terms_title || categoryTerms?.terms_category_name || '위치기반 서비스 이용약관'}
        </h1>
