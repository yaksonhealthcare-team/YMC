...
const DevPage = () => {
  ...
  return (
    <>
      ...
      <div className="p-4 border-t">
        {'GenderSelect: '}
        <GenderSelect value={selectedGender} onChange={(gender) => setSelectedGender(gender)} />
        <div className="mt-2">
          <GenderSelect value={selectedGender} onChange={(gender) => setSelectedGender(gender)} disabled />
        </div>
      </div>
      <div className="p-4 border-t">
        <Button variantType="primary" sizeType="s" onClick={() => navigate('/logout')}>
          로그아웃
        </Button>
      </div>
    </>
