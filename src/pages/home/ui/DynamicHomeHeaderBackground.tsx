...
const ContentNode = memo(({ content }: { content: ReactNode }) => {
  return <div className="relative w-full bg-cover bg-center px-5">{content}</div>;
});
    ...
    return (
      ...
      >
        ...
        <div className="relative w-full bg-white" style={{ marginTop: '-1px' }}>
          ...
          {contents && contents.map((item, i) => <ContentNode key={i} content={item} />)}
        </div>
        ...
        <div className="relative w-full mb-1" style={{ marginTop: '-1px' }}>
          <svg
            ...
            preserveAspectRatio="xMidYMid slice"
          >
            <path d="M335 6C335 17.0457 326.046 26 315 26H20C8.95431 26 0 17.0457 0 6V0H335V6Z" fill="white" />
          </svg>
        </div>
      </div>
    );
...
const DynamicSquareContainer = ({ children, className = '' }: DynamicSquareContainerProps) => {
