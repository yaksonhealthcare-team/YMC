...
const ReviewFormPage = () => {
  ...
  useEffect(() => {
    setHeader({
      ...
      title: '만족도 작성',
      left: (
        <button onClick={() => navigate(-1)} className=" rounded" aria-label="뒤로가기">
  ...
  const handleRatingChange = (rs_idx: string, newGrade: ReviewRating['rs_grade']) => {
    setRatings((prev) => prev.map((rating) => (rating.rs_idx === rs_idx ? { ...rating, rs_grade: newGrade } : rating)));
  };
  ...
  const ReviewSectionComponent = ({ section, rating }: { section: ReviewSection; rating: ReviewRating }) => {
    const displayName = section.sc_name.endsWith('관리') ? section.sc_name : `${section.sc_name} 관리`;

    return (
      <div className="flex flex-col gap-4">
        <div className="text-gray-700 text-base font-semibold">{displayName}는 어떠셨나요?</div>
  ...
  return (
    <div className="flex flex-col pb-[94px]">
        ...
        <div className="flex flex-col gap-5 bg-white rounded-[20px]">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-gray-500 text-sm font-medium">2024년 10월 26일 (토)</span>
            </div>
            <h2 className="text-gray-700 text-lg font-bold">약손명가 강남구청역점</h2>
          </div>
          <p className="text-gray-500 text-sm font-medium">K-beauty 연예인 관리 A 코스</p>
        </div>
      </div>
      ...
      <div className="px-5 py-8 flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-4">
          <div className="text-gray-700 text-base font-semibold">종합적인 평가를 작성해주세요. (선택)</div>
          <TextArea
            value={review}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar touch-pan-x">
            <label className="shrink-0 w-20 h-20 border border-gray-200 rounded-lg flex flex-col items-center justify-center cursor-pointer">
              ...
              <span className="text-gray-400 text-xs font-medium mt-1">{images.length} / 8</span>
            </label>
            {images.map((image, index) => (
              <div key={image.id} className="relative w-20 h-20 rounded-lg border border-gray-100">
                <Image
                  src={image.preview}
                  alt={`리뷰 이미지 ${index + 1}`}
      ...
      <FixedButtonContainer className="bg-white">
        <Button variantType="primary" sizeType="l" disabled={!isFormValid} onClick={handleSubmit} className="w-full">
