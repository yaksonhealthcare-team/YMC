        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-primary text-base font-bold">{review.visit}회차</span>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex gap-1.5">
              {Object.entries(review.grade).map(([key, value]) => (
                <div key={key} className="bg-tag-redBg rounded-full py-[3px] px-2">
            ...
            {review.content && <p className="text-gray-900 text-sm leading-[23.52px]">{review.content}</p>}

            {review.imageUrls && review.imageUrls.length > 0 && (
              <div className="w-full flex gap-2 overflow-x-auto no-scrollbar">
                {review.imageUrls.map((image, index) => (
                  <div key={index} className="shrink-0 w-20 h-20 rounded-lg border border-gray-100">
                    <Image
                      src={image}
                      alt={`리뷰 이미지 ${index + 1}`}
