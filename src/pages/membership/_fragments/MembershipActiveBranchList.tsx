...
export const MembershipActiveBranchList = ({ onBranchSelect, brandCode }: Props) => {
  ...
  // 사용 가능한 지점 목록
  const availableBranches: BranchInfo[] = location.state?.availableBranches ?? [];
  ...
  const filteredBranches = useMemo(() => {
    ...
    if (availableBranches.length === 0) {
      ...
      if (currentBrandCode) {
        return (user.brands as BranchInfo[]).filter((brand) => brand.brandCode === currentBrandCode);
      }
    ...
    return (user.brands as BranchInfo[]).filter((brand) =>
      availableBranches.some(
        (availableBranch: BranchInfo) =>
          availableBranch.b_idx === brand.b_idx && (currentBrandCode ? brand.brandCode === currentBrandCode : true)
      )
    );
