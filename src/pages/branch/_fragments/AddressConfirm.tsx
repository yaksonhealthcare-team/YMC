...
const AddressConfirm = () => {
  ...
  useEffect(() => {
    ...
    if (routeLocation.state?.selectedLocation) {
      const { address: locationAddress, coords } =
      ...
      const matchedBookmark = bookmarks.find(
        (bookmark) =>
          ...
          (Math.abs(parseFloat(bookmark.lat) - coordinates.latitude) < 0.0001 &&
            Math.abs(parseFloat(bookmark.lon) - coordinates.longitude) <
              0.0001),
      )
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex flex-col px-5 mt-[64px]">
        {routeLocation.state?.selectedLocation?.name && (
          <div className="font-b text-[18px] text-gray-900 mb-2">
            {routeLocation.state.selectedLocation.name}
          </div>
        )}
      ...
      <div
        ...
        tabIndex={0}
        aria-label={
          isBookmarked ? "자주 쓰는 주소에서 삭제" : "자주 쓰는 주소로 등록"
        }
      >
        <div
          className={`text-[14px] font-m ${isBookmarked ? "text-[#F37165]" : "text-gray-400"}`}
        >
          자주 쓰는 주소로 등록
        </div>
        {isBookmarked ? (
