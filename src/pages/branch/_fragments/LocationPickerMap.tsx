...
const LocationPickerMap = () => {
    ...
    navigate('/branch/location/confirm', {
      state: {
        selectedLocation: {
          ...
          coords: center,
          name: selectedBranch?.name || routeLocation.state?.selectedLocation?.name || ''
        }
      }
    });
  };
  ...
  return (
    <div className={'relative w-full h-full'}>
      ...
      />
      <LocationSelectorPin
        className={'absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-[100%] pointer-events-none'}
      />
      <div className={'absolute bottom-0 rounded-t-2xl bg-white w-full z-[300] pt-6 pb-8'}>
        {address.road.length > 0 && <p className={'px-5 font-b'}>{address.road}</p>}
        {address.jibun.length > 0 && <p className={'px-5 text-14px text-gray-500 mt-2'}>{address.jibun}</p>}
        <div className={'w-full h-[1px] bg-gray-50 mt-6'} />
        <div className={'w-full px-5 mt-3 flex gap-2'}>
          <Button variantType={'primary'} className={'flex-1'} onClick={handleSetLocation}>
            {'이 위치로 설정'}
          </Button>
        </div>
      </div>
