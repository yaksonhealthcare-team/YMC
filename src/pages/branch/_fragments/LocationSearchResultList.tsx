    ...
    <ul className={'px-5 divide-y divide-gray-100'}>
      {locations.map((location, index) => (
        <li key={location.csab_idx ?? index} className={'py-4'}>
          <div className={'flex items-center gap-2'} onClick={() => onClick(location)}>
            <PinIcon className={'w-4 h-4'} />
            <div className="flex-1">
              <p className={'font-b text-14px'}>{location.name || location.address}</p>
              <p className={'text-12px text-gray-500 mt-1'}>{location.name ? location.address : ''}</p>
            </div>
            {type === 'saved' && onDelete && (
              <button
                onClick={(e) => {
