      ...
      <div className="w-full flex flex-col justify-between py-1">
        <div className="flex justify-between">
          <div>
            ...
            <address className="font-r text-14px text-start not-italic ">{branch.address}</address>
          </div>
          <button
            type="button"
            onClick={(e) => {
            ...
            className={clsx('rounded-lg px-1 h-fit', 'transition-transform hover:scale-110')}
            aria-label={`${branch.name} 지점 ${isFavorite ? '즐겨찾기 해제' : '즐겨찾기 추가'}`}
            aria-pressed={isFavorite}
          >
            {isFavorite ? <HeartEnabledIcon aria-hidden="true" /> : <HeartDisabledIcon aria-hidden="true" />}
          </button>
        </div>
      </div>
    </div>
    ...
    return (
      <div className={clsx('flex flex-col h-full', className)}>
        <div className="pt-[16px] px-5 flex-none bg-white">
          <p className="font-m text-14px text-gray-700" role="status" aria-live="polite">
            {'총 '}
  ...
  return (
    <div className={clsx('flex flex-col h-full', className)}>
      <div className="flex-1 overflow-y-auto">
        ...
        <ul className="divide-y px-5 pb-[82px]" role="listbox" aria-label="지점 목록" tabIndex={0}>
