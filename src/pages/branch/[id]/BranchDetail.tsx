...
const BranchDetail = () => {
  ...
  const { data: branch, isLoading } = useBranch(id!, currentLocation || undefined, {
    ...
    <div className={'relative flex-grow w-full bg-system-bg overflow-x-hidden'}>
      <div className={'flex flex-col gap-3'}>
        <Suspense fallback={<LoadingIndicator className="h-20" />}>
          <BranchHeader branch={branch} onShare={handleShare} onBack={handleBack} />
      ...
      <Suspense fallback={<LoadingIndicator className="h-20" />}>
        <BranchTabs selectedTab={activeTab} onChangeTab={(tab: BranchDetailTab) => setActiveTab(tab)} branch={branch} />
      </Suspense>
      <div className={'h-20'} />
      <Suspense fallback={<LoadingIndicator className="h-20" />}>
        <BranchActions branch={branch} onBookmark={handleBookmark} onReservation={handleReservation} />
      </Suspense>
    </div>
  );
};
