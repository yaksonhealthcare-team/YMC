...
import { SearchFloatingButton } from '@/shared/ui/button/SearchFloatingButton';
...
import { Branch as BranchType } from '@/entities/branch/model/Branch';
...
const Branch = () => {
  ...
  useEffect(() => {
    // 최초 렌더링 시 제외
    if (prevSelectedLocationRef.current !== selectedLocation && selectedFilter.enabled) {
  ...
  useEffect(() => {
    ...
    if (routeLocation.state?.from === '/branch/location' || routeLocation.state?.from === '/branch/location/picker') {
  ...
  const isLoading = locationLoading || (selectedFilter.enabled && branchesLoading) || isFetching;
  ...
  const handleFilterChange = (newFilter: { brand: FilterItem | null; category: FilterItem | null }) => {
    // 선택한 필터가 변경된 경우에만 상태 업데이트 및 API 호출
    const brandChanged = selectedFilter.brand?.code !== newFilter.brand?.code;
    const categoryChanged = selectedFilter.category?.code !== newFilter.category?.code;
  ...
  useEffect(() => {
    setHeader({
      component: (
        <div>
          <div className={'w-full justify-between flex px-5 py-3.5 bg-white h-12 gap-4'}>
            <button onClick={handleNavigateToBack}>
              <CaretLeftIcon className={'w-5 h-5'} />
            </button>
            <button className={'flex gap-2 items-center'} onClick={handleNavigateToLocationSettings}>
              <p className={'font-sb text-14px overflow-ellipsis line-clamp-1'}>{displayAddress()}</p>
  ...
  return (
    <div className={clsx('flex flex-col w-full h-full', screen === 'list' ? 'pt-[48px]' : 'pt-[0px]')}>
      {screen === 'list' ? (
        <BranchFilterList
          branches={branches}
          onIntersect={() => {
      ...
      <div
        className={clsx(
          ...
          screen === 'map' && selectedBranch ? 'bottom-[calc(8rem+2rem)]' : 'bottom-10'
        )}
      >
