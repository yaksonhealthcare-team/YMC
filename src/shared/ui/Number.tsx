...
export const Number = (props: NumberProps) => {
  const { count, disabled, minimumCount, maximumCount = 999, onClickMinus, onClickPlus } = props;
  ...
  return (
    ...
    >
      ...
      >
        <MinusIcon className={clsx('w-4 h-4', { 'text-gray-300': isMinusDisabled })} aria-hidden="true" />
      </button>
      <span
        id="current-count"
        className={clsx('w-8 h-6 font-sb text-14px text-gray-700 border-y flex justify-center', {
      ...
      <button
        className={clsx('flex justify-center items-center w-6 h-6 border rounded-r', '', {
