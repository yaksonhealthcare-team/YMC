    ...
    <div>
      <div className="flex flex-col gap-1">
        <CustomTextField
          ...
          placeholder="비밀번호 입력"
          state={passwordError ? 'error' : password && !validatePassword(password) ? 'error' : 'default'}
        ...
        {!passwordError && password && (
          <span className={`text-12px ml-2 ${validatePassword(password) ? 'text-success' : 'text-error'}`}>
            10-20자, 영문 대/소문자, 숫자, 특수문자 포함
          </span>
        )}
        {!passwordError && !password && (
          <span className="text-12px text-gray-400 ml-2">10-20자, 영문 대/소문자, 숫자, 특수문자 포함</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1 mt-6">
        ...
        {!passwordConfirmError && password && passwordConfirm && (
          <span className={`text-12px ml-2 ${password === passwordConfirm ? 'text-success' : 'text-error'}`}>
